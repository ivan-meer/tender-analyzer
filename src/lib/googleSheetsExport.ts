...
/**
 * Creates a structured Google Spreadsheet with multiple sheets (Tabs):
 * 1. Спецификация и ТЗ
 * 2. Риски Договора
 * 3. Паспорт Закупки и Логистика
 */
export async function exportReportToGoogleSheets(
  ...
  const title = `Закупка - Таблица ТЗ и Рисков: ${result.summary.projectName || result.summary.procurementTitle || 'Тендер 223-ФЗ'}`;
  ...
  const specRows: any[][] = [
    ['№', 'Наименование позиции', 'Количество', 'ОКПД2 / КТРУ', 'Параметры и Характеристики', 'Требования ТЗ'],
  ];
  ...
  // 3. Build rows for Tab 2: "Риски Договора"
  const riskRows: any[][] = [
    ['№', 'Критичность', 'Заголовок риска', 'Пункт договора', 'Цитата из документа', 'Разъяснение эксперта', 'Рекомендация юриста'],
  ];
  ...
  if (result.contractRisks && result.contractRisks.length > 0) {
    ...
    riskRows.push(['1', 'БЕЗОПАСНО', 'Критический рисков в договоре не выявлено', '—', '—', 'Проект договора соответствует стандарту', '—']);
  }
  ...
  // 5. Write data into Google Sheet
