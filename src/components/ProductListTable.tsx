              ...
              filteredProducts.map((item, idx) => (
                <tr key={item.id || idx} className="hover:bg-indigo-50/30 transition-colors align-top">
                  ...
                  <td className="py-3.5 px-4 space-y-2">
                    ...
                    {/* Extracted Structured Parameters Grid */}
                    {item.parameters && item.parameters.length > 0 && (
                      <div className="pt-1.5">
                        <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                          ...
                          <span>Извлеченные параметры из ТЗ:</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {item.parameters.map((param, pIdx) => (
                            <div 
                              key={pIdx} 
                              className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 border border-slate-200 rounded-md text-[11px]"
                            >
                              <span className="font-semibold text-slate-500">{param.name}:</span>
