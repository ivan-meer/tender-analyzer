...
export const DeliveryLogisticsCard: React.FC<DeliveryLogisticsCardProps> = ({ deliveryInfo }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-white dark:bg-slate-900 border-2 border-indigo-200 dark:border-indigo-900/60 rounded-3xl shadow-xs overflow-hidden transition-all duration-200"
    >
      {/* Top Banner Header */}
      <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-slate-900 text-white p-5 sm:p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-500/30 text-indigo-200 border border-indigo-400/30">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="px-3 py-1.5 bg-white/10 rounded-xl border border-white/10 text-xs font-bold text-indigo-100 flex items-center gap-1.5 shadow-2xs">
              <Clock className="w-3.5 h-3.5 text-amber-300" />
              <span>Адресов поставки: {addresses.length}</span>
            </div>
            {deliveryInfo.deliveryScheduleNotice && (
              <div className="px-3 py-1.5 bg-emerald-500/20 border border-emerald-400/30 rounded-xl text-xs font-bold text-emerald-200 flex items-center gap-1.5 shadow-2xs">
      ...
      <div className="p-5 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* Column 1: Сроки поставки и порядок (6 cols on lg) */}
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-slate-50 dark:from-indigo-950/40 dark:to-slate-800/80 border-2 border-indigo-200 dark:border-indigo-800/80 rounded-2xl p-4 space-y-2 relative overflow-hidden shadow-2xs">
              <div className="absolute top-0 right-0 transform translate-x-2 -translate-y-2 w-16 h-16 bg-indigo-100/50 dark:bg-indigo-900/30 rounded-full blur-lg pointer-events-none"></div>
              <div className="text-[11px] font-extrabold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider flex items-center gap-1">
            ...
            {deliveryInfo.deliveryScheduleNotice && (
              <div className="bg-amber-50/80 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-2xl p-3.5 flex items-start gap-3 shadow-2xs">
                <AlertTriangle className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                <div className="text-xs space-y-0.5">
                  <span className="font-extrabold text-amber-900 dark:text-amber-200 block">Порядок графика заявок:</span>
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="space-y-2.5">
              {addresses.map((addr, idx) => (
                <div 
                  key={idx} 
                  className="group bg-slate-50 dark:bg-slate-800/80 hover:bg-rose-50/50 dark:hover:bg-rose-950/30 border border-slate-200 dark:border-slate-700 hover:border-rose-300 dark:hover:border-rose-700 rounded-2xl p-3.5 transition-all flex items-start justify-between gap-3 shadow-2xs"
                >
                  <div className="flex items-start gap-3">
                    <div className="p-2 bg-rose-100 dark:bg-rose-950 text-rose-700 dark:text-rose-300 rounded-xl shrink-0 mt-0.5 group-hover:scale-105 transition-transform">
                      <Navigation className="w-4 h-4" />
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 bg-rose-100 dark:bg-rose-950 text-rose-800 dark:text-rose-300 rounded-md">
                          Пункт №{idx + 1}
                        </span>
                  ...
                  <button
                    ...
                    className="p-2 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-white dark:hover:bg-slate-700 rounded-xl border border-transparent hover:border-slate-200 dark:hover:border-slate-600 transition-all cursor-pointer shrink-0"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-slate-200 dark:border-slate-800">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-4 space-y-1.5 shadow-2xs">
          ...
          {/* Risk Warning Notice */}
          <div className="bg-rose-50/80 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 rounded-2xl p-4 space-y-1.5 shadow-2xs">
            ...
            <p className="text-[11px] text-rose-800 dark:text-rose-300 font-medium leading-tight">
