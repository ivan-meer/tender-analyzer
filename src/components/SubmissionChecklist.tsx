  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xs space-y-6 transition-colors duration-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-4 sm:p-5 space-y-3 shadow-2xs">
          ...
          <div className="space-y-2.5 text-xs">
            <div className="flex items-start gap-2 text-slate-700 dark:text-slate-300">
              ...
              <div className="font-medium">
                <strong className="text-slate-900 dark:text-white font-bold">Таблица запросов:</strong> Направить запрос по участию в общую таблицу запросов.
              </div>
            </div>

            <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-700 p-3 rounded-xl text-slate-700 dark:text-slate-300 flex items-start gap-2 shadow-2xs">
              <CreditCard className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5" />
              <div className="font-medium">
                <strong className="text-slate-900 dark:text-white font-bold">ЭТП / Аккредитация:</strong> {check.etpAccreditationNotice || 'Если закупка платная или нужна аккредитация — обязательно написать в ЛС тендерному специалисту с ссылкой и указанием компании!'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-4 sm:p-5 space-y-3 shadow-2xs">
          ...
          <div className="space-y-2.5 text-xs text-slate-700 dark:text-slate-300">
            <div className="flex items-start gap-2">
              ...
              <div>
                <strong className="text-slate-900 dark:text-white font-bold">Состав файлов для подачи:</strong>
                <ul className="list-disc pl-4 mt-1 space-y-1 text-slate-600 dark:text-slate-400">
                  {check.requiredFilesStructure && check.requiredFilesStructure.length > 0 ? (
                    check.requiredFilesStructure.map((file, idx) => (
                      <li key={idx} className="text-slate-800 dark:text-slate-200 font-bold">{file}</li>
                    ))
                  ) : (
            ...
            <div className="p-3 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-300 shadow-2xs font-medium">
              <strong className="text-slate-900 dark:text-white font-bold">Формы организатора:</strong> Менеджер заполняет ВСЕ рекомендуемые формы к подаче, <u className="decoration-indigo-500">КРОМЕ описи вложений</u>.
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">{check.formsRequirement}</p>
            </div>
          </div>
        </div>

        {/* Step 3: PP RF 1875 National Regime */}
        <div className={`border rounded-2xl p-4 sm:p-5 space-y-3 shadow-2xs ${check.pp1875Applies ? 'bg-amber-50/60 dark:bg-amber-950/40 border-amber-200 dark:border-amber-900/60' : 'bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700/80'}`}>
          <div className="flex items-center gap-2 font-bold text-xs text-slate-800 dark:text-slate-200 border-b border-slate-200/80 dark:border-slate-700/60 pb-2.5">
          ...
          <div className="space-y-2 text-xs">
            {check.pp1875Applies ? (
              <div className="space-y-2">
                <span className="inline-block px-2.5 py-1 rounded-full bg-amber-100 dark:bg-amber-900/80 text-amber-800 dark:text-amber-200 font-extrabold text-[11px] border border-amber-200 dark:border-amber-700">
                  ПП РФ № 1875 Применяется!
                </span>
                <p className="text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
                  Обязательно указывать <strong className="text-slate-900 dark:text-white font-bold">реестровые номера</strong> и <strong className="text-slate-900 dark:text-white font-bold">количество баллов</strong> по каждой российской позиции в заявке.
                </p>
                <div className="text-[11px] text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900/90 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xs font-medium">
                  {check.pp1875Details}
                </div>
              </div>
            ) : (
                ...
                <ul className="list-disc pl-4 space-y-1 text-slate-700 dark:text-slate-300 font-medium">
                  {check.accountingItems && check.accountingItems.length > 0 ? (
                    check.accountingItems.map((item, i) => (
                      <li key={i} className="font-bold text-slate-900 dark:text-white">{item}</li>
                    ))
                  ) : (
                    <>
